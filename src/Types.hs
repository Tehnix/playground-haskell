...
emptySymbolTable = SymTable { symTableParent = Nothing, symTableAttributes = Map.empty }

incLineNo :: (Num a, HasCurrentNo t a) => a -> t -> t
incLineNo n = over currentNo (+ n)

modifyLineNo :: (HasLineNo env (TVar a), MonadIO m, MonadReader env m) => (a -> a) -> m ()
modifyLineNo f = do
  env <- ask
  liftIO $ atomically $ modifyTVar' (env^.lineNo) f

modifySteps :: (HasSteps env (TVar a), MonadIO m, MonadReader env m) => (a -> a) -> m ()
modifySteps f = do
  env <- ask
